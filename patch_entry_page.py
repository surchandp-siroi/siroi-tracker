...
with open('src/pages/EntryPage.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Add States
if "const [stagedItems, setStagedItems] = useState<EntryItem[]>([]);" not in content:
...
processFileRegex = re.compile(r"const processFile = async \(file: File\) => \{.*?\n  \};\n\n  const handleFileUpload", re.DOTALL)

new_processFile = """const processFile = async (file: File) => {
...
content = processFileRegex.sub(new_processFile, content)

# 3. Add handleBulkSubmit
deleteRegex = re.compile(r"setIsDeleting\(false\);\n      \}\n  \};")
handleBulkSubmitStr = """setIsDeleting(false);
      }
  };
...
content = deleteRegex.sub(handleBulkSubmitStr, content)

with open('src/pages/EntryPage.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Patched EntryPage.tsx")
